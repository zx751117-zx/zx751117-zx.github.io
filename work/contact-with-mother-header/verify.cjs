...
for(const file of pages){const $=c.load(fs.readFileSync(file,'utf8'));
 ...
 if(file!=='contact_cn.html'){
  const contact=$('#module316FlBtn').attr('href');assert.equal($('#dongying-header').length,0);assert.equal($('#logoImg').attr('alt'),'北京盈創天地科技有限公司');
...
const window={location:{href:''}},document={getElementById:id=>id==='contact-form'?form:null,addEventListener:(_,fn)=>fn()};
